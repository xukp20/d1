...
class dLLMDataCollator(DefaultDataCollator):
    def __init__(self, *args, **kwargs):
        ...
        if kwargs['tokenizer'].mask_token_id is None:
            assert 'mask_token_id' in kwargs, 'For dLLM models, pass a mask_token_id or set it equal to tokenizer.mask_token_id'
...
def preprocess_dataset(data, tokenizer, max_length, test_split=0.01):
    ...
    for i in tqdm(range(len(data)), desc='Preprocessing dataset'):
        ...
        trajectory =  f'<reasoning>{data[i]['thinking_trajectories'][0]}</reasoning>\n<answer>{data[i]['attempt']}</answer>' # data[i]['generations'][0].replace('<think>', '<reasoning>').replace('</think>', '</reasoning>')
        prompt = [
        ...
        tokenized_input = tokenizer(inputs, return_tensors='pt', truncation=True, max_length=max_length, padding='max_length').input_ids.squeeze(0)
